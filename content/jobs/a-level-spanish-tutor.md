---
title: "A-Level Spanish Tutor Jobs (Online, UK) | The Degree Gap"
description: "A-Level Spanish requires analytical essays on set works and an individual research. Tutor A-Level Spanish online with The Degree Gap. Enhanced DBS required."
robots: "index, follow"
role_title: "A-Level Spanish Tutor"
h1: "A-Level Spanish Tutor Jobs"
subject: "A-Level Spanish"
level: "A-Level"
subject_page: "/subjects/a-level-spanish-tutor/"
employment_type: "Part-time, self-employed"
work_type: "Online, UK wide"
---

## About the role

The Degree Gap is recruiting experienced A-Level Spanish tutors for online one-to-one teaching.

A-Level Spanish requires analytical essays on set works and an individual research project, alongside translation accuracy beyond GCSE demand.

Students are matched to tutors by one of the co-founders following a consultation with the family. You will receive the student's current attainment, target grade and relevant background before the first session. We recruit for A-Level Spanish on a rolling basis.

## Responsibilities

- Teach analytical essay writing on the set text and film
- Supervise the individual research project
- Deliver translation in both directions to A-Level standard
- Prepare students for spontaneous discussion in the oral examination

## Requirements

Candidates must be able to teach analytical writing in Spanish, not accuracy alone.

- A minimum of two years' teaching or tutoring experience
- Demonstrable experience teaching A-Level Spanish at this level
- An enhanced DBS certificate, verified before any student is assigned
- The right to work in the UK
- A reliable internet connection and a quiet teaching environment

## Terms

The role is self-employed and part-time, with hours set by the tutor. Students are allocated according to your stated availability and subject coverage. Sessions are scheduled and paid through the platform, so tutors do not invoice families directly and are not required to advertise or source their own students. A named contact is available for scheduling and safeguarding matters.

## Selection process

All applicants are interviewed by a co-founder before appointment. The interview covers subject knowledge, teaching approach, and experience of supporting students working below their target grade. We appoint a small proportion of applicants.

## How to apply

Contact us by WhatsApp or email with a summary of your teaching experience and the subjects and levels you cover.
