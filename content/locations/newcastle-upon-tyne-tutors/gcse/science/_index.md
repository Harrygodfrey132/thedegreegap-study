---
title: "GCSE Science Tutors for Newcastle upon Tyne | The Degree Gap"
nav_title: "Science"
description: "Online GCSE Science tutors covering Newcastle upon Tyne. One method for the calculations in all three sciences, one-to-one on Lessonspace, from £37/hr."
layout: "level-subject"
location: "Newcastle upon Tyne"
level: "GCSE"
subject: "Science"

hero_h1: "Online GCSE Science tutors for Newcastle upon Tyne students stuck on the calculation questions"
hero_lead: "Fine in Maths, blank in Physics, and nobody can say why the same sum stops working in a different room. Our Newcastle upon Tyne GCSE Science tutors teach one method for all three sciences, one-to-one online over Lessonspace, until it travels."

angle_eyebrow: "ONE METHOD, THREE CLASSROOMS"
angle_heading: "Why the Same Calculation Works in Maths and Fails in Physics"
angle_body_1: "Combined Science comes home as one grade with a hyphen in it, taught by three teachers in three rooms, and the calculation questions run through all of them. Rearranged equations in Physics, moles and concentration in Chemistry, percentage change and magnification in Biology: each is taught its own way, none of them the way Maths did it, so a student who is fine in Maths on Tuesday goes blank on the same step in Physics on Thursday. Add units, standard form and significant figures, and the marks that bleed are not science marks at all. Newcastle College names science beside Maths and English in its T Level requirements, and the sixth forms set their own bar for the A-Level sciences. Expensive marks to lose."
angle_body_2: "The first session pulls one recent paper apart into the three sciences and then into question types, and the calculation questions usually land in one pile with the same missing step. The tutor teaches that step once, the same way for all three subjects, written out on the Lessonspace whiteboard with the units carried through every line, then drills it inside real exam questions until it holds whichever room it turns up in. Where the tier is still open in Year 10, the same paper settles it. St Mary's, Gosforth Academy, the Royal Grammar School and the other sixth forms each publish their own requirements for the A-Level sciences, revised between years."
angle_image: "/images/chemistry-lab-glassware.jpg"
angle_image_alt: "A GCSE Science student working through a calculation question with an online tutor"
angle_stat_from: "4-4"
angle_stat_to: "6-6"
angle_stat_detail: "Where a term of weekly one-to-one Combined Science sessions is typically aimed once the calculation step holds in all three subjects."

schools:
  - "St Mary's Catholic School"
  - "Royal Grammar School Newcastle"
  - "Gosforth Academy"
  - "Walbottle Academy"
  - "Sacred Heart Catholic High School"
  - "Jesmond Park Academy"
  - "Dame Allan's Schools"

steps_heading: "Start online GCSE Science tutoring in Newcastle upon Tyne in 3 steps"
steps_lead: "Profiles inside a day. Say whether it is triple or combined, and which of the three sciences has the calculation questions that go blank."
steps:
  - title: "Share your goals"
    body: "Year group, school, board, tier, and whether your child sits triple or combined. Then the useful question: is the Maths grade fine while the Physics or Chemistry one is not, because that pattern points straight at the calculations."
  - title: "Meet your match"
    body: "Two or three profiles the next day, chosen on the board and on the science doing the damage rather than on Science in general, with a free 15-minute video meeting first. Ask for a tutor who covers Maths too if the pattern fits."
  - title: "Start the lessons"
    body: "A standing weekly hour, one-to-one, online on Lessonspace, our shared whiteboard, with the working and the units written on screen and every lesson recorded. One method for three subjects, then real questions. From £37 an hour, no contract."

faqs:
  - q: "How does GCSE Science performance in Newcastle upon Tyne compare to the national average?"
    a: "Across the city as a whole, results sit below the national picture, and the spread between schools is wider than the headline. That average tells you almost nothing about one child, though. What matters is your child's last Science paper against the tier they are entered for. For many Newcastle students the job is the shift from foundation to higher, or a secure 4-4 that keeps the college and T Level routes open. For others it is the 7 that the A-Level sciences at the sixth forms ask for. The first session works out which brief is in front of it, and the plan follows from there."
  - q: "When is the best time to start GCSE Science tutoring in Year 11?"
    a: "September to February, and the earlier end of that. The winter mock produces the predicted grades a Newcastle upon Tyne Year 11 carries into sixth-form applications, with Gosforth Academy closing its list in January and Newcastle Sixth Form College setting a requirement for each science, so work that lands before the mock counts twice. Science also has a lot of content to hold, and a term is enough to fix the calculation step and revise one science properly. A start in April or May still helps: a short block on the one science doing the damage, timed papers, and the six-mark questions."
  - q: "How do you vet your GCSE Science tutors?"
    a: "Around 3% of the tutors who apply end up on the platform. The stages run in order: a written application, a test of the subject knowledge, then a structured interview where the applicant has to take an idea like moles or rearranging an equation and make it land with someone who does not yet have it. That last stage removes most people, because knowing Chemistry and explaining Chemistry are different skills. Harry and Joe run those interviews themselves, and references are taken at the end. Newcastle families then see two or three profiles and choose."
  - q: "What happens if the tutor is not the right fit for my child?"
    a: "You tell us and we change it, at no cost. There is a free 15-minute video meeting with the recommended tutor before any paid session, and plenty of families rule someone out at that stage. If it takes a first lesson to know, say so within a day or so and the next tutor is arranged without a charge and without your child having to explain themselves. The network is deep enough that a second match is rarely a compromise, and for Science we can also switch to a tutor who leads on a different one of the three."
  - q: "Can I sit in on the GCSE Science tutoring session?"
    a: "Yes, and most tutors welcome it for the first one. It is useful for a parent to see how the working goes down on the Lessonspace whiteboard and how the tutor checks the units line by line. After that first session most parents step back, because a fifteen-year-old says 'I don't get it' more readily without an audience, and that sentence is where the progress starts. You still see everything: every session is recorded, and the notes after each one say what was covered and what the homework is. Newcastle upon Tyne parents tend to watch the recording instead."
# FAQ picks: A03, C01, C06, E01, G02

reviews:
  - "Augusta|Parent of GCSE Student|The support given to my son by The Degree Gap's tutors was fabulous and he passed all his GCSE subjects. The Degree Gap provide experienced and organised tutors. To improve my son's grades, the tutors provided resources, targeting specific areas of difficulties. The website is easy to use and effective, I had no disappointment in scheduling lessons for my son. Tutors were polite and on time for their lessons."
  - "Sureet|Parent of A-Level Student|Haneet has really helped my son in navigating his A level Physics all whilst being kind and supportive."
  - "Sorland|Grandparent|My Granddaughter uses the Degree Gap tutoring services across a few subjects. She is not only improving academically but really enjoys the setup and structure too. I would recommend to anyone."
  - "Heidi|Parent|Emir, is simply amazing with our daughter. He 'gets' her totally, her sessions are helping her with her school work so much. We can't thank him enough ! Harry and Joe are always on hand should you need anything. Degreegap is an excellent company to work with, lots of excellent tutors avaliable to teach different subjects, great availability, very reasonably priced. Would highly recommend."
  - "Lisa|Parent of Two Students|Joe and Harry at The Degree Gap have exceeded all my expectations.  They have gone above and beyond to pair both my sons with the appropriate tutors, all of whom have been exceptional.  Both my sons exceeded well above their expected grades in their GCSE's and A Levels and got into their first choice Universities. I cannot recommend them highly enough."

sitemap:
  priority: 0.7
  changefreq: monthly
---
