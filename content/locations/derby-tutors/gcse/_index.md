---
title: "Derby GCSE Tutors | One-to-One Support | The Degree Gap"
description: "Derby GCSE tutors who close the mock-to-predicted gap and rebuild exam technique before Year 11 finishes. Maths, English, Sciences, matched per exam board."
layout: "subject"
location: "Derby"
level: "GCSE"
grade_from: "E"
grade_to: "B"
grade_stat: "A real grade E to grade B GCSE jump from a student we worked with."

hero_heading_line1: "Derby GCSE Tutors Who Catch What School Misses"
hero_heading_line2: ""
hero_lead: "Year 11 mocks landing well below predicted? Our Derby GCSE tutors trace the gap back to where it started and fix exam technique before the next round."

first_lesson_eyebrow: "INSIDE A YEAR 11 RESCUE"
first_lesson_heading: "What a Derby GCSE Tutor Spots in Year 10 Mocks"
first_lesson_context: "But by Year 11, most Derby gaps trace back to specific topics that quietly compounded through Year 8 and 9. A Derby GCSE tutor finds that root in week one, and the GCSE tutoring plan builds back from there."
first_lesson_quote: "Has definitely helped me increase my grades from an E to a B."
first_lesson_quote_name: "Keira"
first_lesson_quote_role: "GCSE Student"
first_lesson_quote_grade: "Grade E to Grade B"

tutor_strip_heading: "Derby GCSE tutors who know what Year 10 hides"
tutor_strip_body: "Our Derby GCSE tutors have worked with Year 10 and 11 students across Chellaston, Ecclesbourne, Bemrose and Allestree. Browse the roster, or we'll make the introduction."

pathways_heading: "Where Derby Year 11s End Up"
pathways_lead: "Three routes most Derby GCSE tutoring students take after results day."
pathways:
  - title: "Sixth Forms"
    body: >
      Chellaston Academy Sixth and Ecclesbourne School lead the local sixth-form performance, with Littleover and Allestree Woodlands sitting alongside them. A GCSE Maths tutor or GCSE English tutor in Year 11 lifts the predicted grade towards what these sixth forms actually ask for.
  - title: "Apprenticeships"
    body: >
      Rolls-Royce at Sinfin and Toyota at Burnaston run Derby's biggest Year 11 engineering apprenticeship intakes, with Alstom adding rail-engineering routes from Litchurch Lane. Most have published minimum-grade requirements in Maths and English, with GCSE Sciences tutoring often the difference for the technical route.
  - title: "Further Education"
    body: >
      Derby College runs the city's main BTEC and T Level routes from the Roundhouse Campus in Pride Park and the Joseph Wright Centre in town. One-to-one GCSE tutoring in Year 11 lifts students past the minimum Level 3 entry threshold, especially for the engineering and digital T Levels feeding the Rolls-Royce supply chain.

reviews:
  - "Keira|GCSE Student|Has definitely helped me increase my grades from an E to a B."
  - "Omo|Parent of GCSE Student|My son, who is in Year 10, really struggled with English and was completely disengaged from the subject. After six months of tutoring, he has improved from a grade 5 to a 6/7. He now looks forward to his lessons. Through one-to-one tutoring he has learned effective strategies that help him approach English in a way that resonates with him."
  - "Augusta|Parent of GCSE Student|The support given to my son by The Degree Gap's tutors was fabulous and he passed all his GCSE subjects. They provided experienced and organised tutors. Tutors provided resources targeting specific areas of difficulty. The website is easy to use and tutors were polite and on time."
  - "Ellen|Parent of GCSE Student|My daughter was struggling with confidence in Maths GCSE and we decided to get a tutor. We were given a selection of tutors to choose from and told that if it did not feel like a good match we could try others. Our tutor was a complete star, listened, made it simple, set good homework and gave my daughter her confidence back. Highly recommend."
  - "Chris|Parent of GCSE Student|Very good tutoring for my daughter and good communication with parents. My daughter passed both English and Maths GCSE."
  - "Philippa|Parent of GCSE Student|I was really pleased to discover the Degree Gap when my daughter needed help getting the grades she needed for GCSEs. The team offer a really personalised service and I felt really well looked after from the start. There are lots of tutors to choose from and all subjects are catered for, with great flexibility on timings. Highly recommend."
  - "Bryan|GCSE Student|Helped me a lot and really showed me what I need to improve on."
  - "Jessica|GCSE Student|I was helped with both Maths and English for my GCSEs and passed both subjects with great results, which gave me the foundation and confidence to go on to succeed in my A-levels and complete my degree."
  - "Isabel|Parent of iGCSE Student|The Degree Gap gave our son the support he needed for his iGCSEs, especially in Maths and Economics. Lessons were clear, engaging, and built his confidence. Highly recommended."
  - "Heidi|Parent|Our tutor is simply amazing with our daughter. He understands her completely and her sessions are helping her with her school work so much. We cannot thank him enough."
  - "Nazrul|Parent|My daughter found the tutors were very helpful for her GCSE. Thank you."
  - "Lisa|Parent|The Degree Gap has exceeded all my expectations. They have gone above and beyond to pair both my sons with the appropriate tutors, all of whom have been exceptional. Both my sons exceeded their expected grades in their GCSEs and A Levels and got into their first choice universities. I cannot recommend them highly enough."
  - "Annette|Parent|This company helped us find a tutor for Geography and my son has excelled so much in just a few months. Our tutor has gone above and beyond, demonstrating incredible patience and encouraging my son to believe in himself. He also manages neurodiversity effectively."
  - "Marfa|Student|Very engaging lessons that helped me improve my grade in maths and successfully get admitted to university."
  - "Theresa|Parent|The Degree Gap was fantastic. Their knowledge and guidance helped my son find direction in both his studies and future plans."
  - "Sorland|Grandparent|My granddaughter uses the Degree Gap tutoring services across a few subjects. She is not only improving academically but really enjoys the setup and structure too. I would recommend to anyone."

faq_1_q: "What if my child has fallen significantly behind in their GCSEs?"
faq_1_a: "Common in Year 10 after a rough Year 9. We start by tracing the gap back to where things actually stopped clicking, and the plan builds from there rather than racing to keep pace with current classroom content. Derby GCSE tutors with experience of behind-target students usually rebuild the basics inside two terms."
faq_2_q: "How does online GCSE tutoring in Derby work?"
faq_2_a: "Almost all of it runs online over Lessonspace, our one-to-one platform with an interactive shared whiteboard. Derby families like it because they're matched with the right tutor for the subject and exam board, not just the nearest geographically. Most students settle into it from the first session."
faq_3_q: "My daughter is struggling with GCSE Maths in Derby. What can a tutor actually do?"
faq_3_a: "First session is diagnostic. A Derby GCSE Maths tutor works through a recent past paper to find the topics losing the most marks, usually in algebra rearrangement or the 9-mark problem-solving question. The plan from session two builds back from where the gap actually started, not from the textbook."
faq_4_q: "How much does GCSE tutoring in Derby cost?"
faq_4_a: "From £37 per hour. The rate depends on the subject and the tutor, and it's agreed before anything is booked. Most Derby families settle on weekly hour-long sessions through Year 10 and 11, with an extra session added in the run-up to November and March mocks."

sitemap:
  priority: 0.7
  changefreq: monthly
---
