---
title: "GCSE English Tutors in Wigan | The Degree Gap"
nav_title: "English"
description: "GCSE English tutors covering Wigan for Language and Literature, from a secure grade 5 to the 7 sixth-form A-Levels want. Online, AQA or Edexcel, from £37/hr."
layout: "level-subject"
location: "Wigan"
level: "GCSE"
subject: "English"

hero_h1: "GCSE English Tutors Covering Wigan"
hero_lead: "In a lot of Wigan homes the GCSE English worry sounds the same: the ideas are there in conversation but they don't land on the page under timed conditions. Our Wigan GCSE English tutors build the essay structure and evidence selection that turn a good point into a marked one."

angle_eyebrow: "WHY GCSE ENGLISH IN WIGAN, SPECIFICALLY"
angle_heading: "Where a GCSE English tutor turns Wigan students' ideas into marks"
angle_body_1: "GCSE English in Wigan rarely comes down to reading ability. Two things tend to hold students back. For some it's getting a secure grade 5 across Language and Literature when the timed essay falls apart under the clock. For others it's the jump from a comfortable 6 to the 7 a Winstanley or St John Rigby humanities A-Level looks for. The marks usually leak in the same places: the comparison paper's essay structure, unseen poetry analysis, evidence selection under timing, and the AO5 and AO6 writing technique on Language Paper 1. Good ideas, not yet shaped for the mark scheme."
angle_body_2: "Week one, the tutor reads a recent essay and a marked paper to see where the argument is losing marks, not just where the spelling slips. From there the plan is practical: a repeatable structure for the comparison question, timed paragraphs so the writing holds up in the hall, and the specific evidence a mark scheme rewards. For students aiming at English, History or Psychology A-Level at St John Rigby or Winstanley, where a strong grade in the subject matters, that shift from knowing the text to writing about it is what moves the grade."
angle_image: "/images/handwriting-study-notes-with-highlighters.jpg"
angle_image_alt: "A GCSE English student annotating an unseen poem with a tutor"
angle_stat_from: "5"
angle_stat_to: "7"
angle_stat_detail: "A one to two band move is the usual target across a term of weekly one-to-one GCSE English work."
schools:
  - "The Deanery CE High School"
  - "Standish Community High School"
  - "Hawkley Hall High School"
  - "St John Fisher Catholic High School"
  - "St Peter's Catholic High School Orrell"
  - "Winstanley College"
  - "St John Rigby College"

steps_heading: "Start GCSE English tutoring in Wigan in 3 steps"
steps_lead: "Most Wigan families are matched within 24 hours, often in the run-up to the mocks that fix a predicted grade."
steps:
  - title: "Share your goals"
    body: "Tell us whether it's English Language, Literature or both, the year group and school, the AQA or Edexcel specification and set texts, and the grade you're working towards."
  - title: "Meet your match"
    body: "Within 24 hours you get two or three vetted GCSE English tutors who know the comparison paper and the unseen poetry question. Free 15-minute video meeting before any lesson is booked."
  - title: "Start the lessons"
    body: "Weekly one-to-one over Lessonspace, marking real paragraphs the way examiners do. Many Wigan Year 11s add a second hour in the weeks before mocks. From £37/hr, cancel any time."

faqs:
  - q: "What is the hardest part of GCSE English for Wigan students?"
    a: "For most Wigan students the hardest marks sit in three places: the comparison question, where a clear essay structure decides the grade; the unseen poetry analysis, which rewards a method more than memorised notes; and the AO5 and AO6 writing on Language Paper 1, done well under strict timing. None of it is about reading ability. It's about shaping an argument for the mark scheme and holding technique together in the hall, which is exactly what one-to-one sessions drill."
  - q: "Why is my child struggling with GCSE English?"
    a: "The usual reason is that nobody has taught them to plan an essay and write it under timed conditions, rather than any gap in understanding the text. A student can discuss Macbeth confidently and still lose marks because the paragraph doesn't answer the question or the evidence is thin. The tutor's first job is reading a recent essay to find which of those it is, then rebuilding a structure the student can repeat under pressure, week by week."
  - q: "When is the best time to start GCSE English tutoring in Year 11?"
    a: "September to February is the strongest window, before the March mocks that feed the predicted grades Winstanley and St John Rigby use for conditional sixth-form offers. Starting then leaves time to fix essay structure and timing before it counts. An April or May start still helps, especially as a focused block on one paper or the set texts, but the earlier the technique work begins, the more it compounds."
  - q: "Are your GCSE English tutors qualified teachers or recent graduates?"
    a: "Both, depending on the match. The network includes qualified English teachers, Russell Group graduates, and recent graduates who sat these exact papers not long ago and know the current mark scheme in detail. For a student chasing the top band, a strong recent graduate often has the edge on what examiners are rewarding right now; for one rebuilding confidence, an experienced teacher's pacing can matter more. We match on the fit, not a fixed type."
  - q: "Is GCSE English tutoring in Wigan actually worth the money?"
    a: "Honestly, it depends on the gap. For a student stuck below their target because the writing falls apart under timing, a one or two band lift often unlocks the sixth-form course they want, which more than justifies it. For a student already cruising at their target, the marginal gain is smaller and we'll say so. The consultation call is built to give you the straight answer, including when tutoring isn't what your child needs right now."

reviews:
  - "Omo|Parent of GCSE Student|My son, who is in Year 10, really struggled with English and was completely disengaged from the subject. After six months of tutoring, he has improved from a grade 5 to a 6/7. He now looks forward to his lessons. Through one-to-one tutoring he has learned effective strategies that help him approach English in a way that resonates with him."
  - "Jessica|GCSE Student|I was helped with both Maths and English for my GCSEs and passed both subjects with great results, which gave me the foundation and confidence to go on to succeed in my A-levels and complete my degree."
  - "Bryan|GCSE Student|Helped me a lot and really showed me what I need to improve on."
  - "Philippa|Parent of GCSE Student|I was really pleased to discover the Degree Gap when my daughter needed help getting the grades she needed for GCSEs. The team offer a really personalised service and I felt really well looked after from the start. There are lots of tutors to choose from and all subjects are catered for, with great flexibility on timings. Highly recommend."
  - "Nazrul|Parent|My daughter found the tutors were very helpful for her GCSE. Thank you."

# FAQ picks: G05, A07, A03, E02, C03
sitemap:
  priority: 0.6
  changefreq: monthly
---
