---
title: "A-Level Tutors in Wigan | The Degree Gap"
description: "One-to-one Wigan A-Level tutoring that closes the depth gap behind Year 12, matched to your child's exam board and the grades their UCAS offer needs."
layout: "subject"
location: "Wigan"
level: "A-Level"

hero_heading_line1: "Wigan A-Level Tutors For Students Feeling the Year 12 Jump"
hero_heading_line2: ""
hero_lead: "Our Wigan A-Level tutors close the gap that opens when Year 12 moves faster than a class can slow down for. Most students start ahead of the mocks that set their UCAS predictions, while there's still time to change the number."

grade_from: "C"
grade_to: "A"
grade_stat: "One A-Level student we worked with came back almost two grades up on his Year 13 mock paper."

first_lesson_eyebrow: "A WINSTANLEY YEAR 12 STORY"
first_lesson_heading: "What a Wigan A-Level Tutor Untangles First"
first_lesson_context: "Wigan's sixth-form results are strong, but the college average hides the individual student who coasted through GCSE and then hit Year 12 like a wall. That's where an A-Level tutor starts, not with more content, but with the exact place the depth stopped landing: the evaluation marks in an essay, the multi-step problem that falls apart halfway."
first_lesson_quote: "My son gained almost two grades higher than his year 13 mock paper. His tutor was excellent."
first_lesson_quote_name: "Daljit"
first_lesson_quote_role: "Parent of A-Level Student"
first_lesson_quote_grade: "Grade C to Grade A at A-Level"

tutor_strip_heading: "Wigan A-Level tutors who know where Year 13 marks leak"
tutor_strip_body: "Our Wigan A-Level tutors are graduates of top Russell Group universities, matched to your child's exam board and the way they learn. Most have walked students through the same spec, from Winstanley to St John Rigby. Browse profiles, or let us make the introduction."

pathways_heading: "After A-Levels in Wigan: University, Apprenticeships, Work"
pathways_lead: "Wigan A-Level students head down a few different routes once UCAS clears."
pathways:
  - title: "Universities"
    body: >
      Winstanley and St John Rigby send strong cohorts to Manchester, Leeds, Durham and Liverpool each year, with the higher-tariff students reaching Imperial and UCL. Closer to home, Salford, UCLan and the University of Greater Manchester take a steady share of Wigan A-Level leavers.
  - title: "Degree Apprenticeships"
    body: >
      BAE Systems runs one of the North West's biggest engineering degree apprenticeship schemes within commuting reach, and the University of Greater Manchester offers degree apprenticeships in digital and systems engineering. Wigan and Leigh College brokers others with local employers.
  - title: "Career Pathways"
    body: >
      Food manufacturing anchors the local economy, with Kraft Heinz at Kitt Green a major technical and graduate employer, and Wrightington, Wigan and Leigh NHS one of the borough's largest. Logistics and public services hire steadily alongside them.

reviews:
  - "Daljit|Parent of A-Level Student|My son gained almost two grades higher than his year 13 mock paper. His tutor was excellent."
  - "Serena|A-Level Student|My tutor is a very intuitive and superb teacher. She listened to my specific concerns and bolstered my confidence in answering questions. She taught me how to think critically and helped me through this highly demanding course. I will happily recommend without a second thought."
  - "Alexander|Parent|Degree Gap helped my kid go from Cs to As. They have a great selection of tutors who not only assist with the curriculum but also helped with university choices and application."
  - "Nick|Parent of A-Level Student|Our son has been receiving tuition for Psychology A Level. The tutor has provided a solid base of support outside of school lessons and helped him to improve his latest grades."
  - "Nicolai|A-Level Student|Before joining The Degree Gap, I found it hard to follow the curriculum. The sessions helped everything click. Lessons were calm, focused, and easy to understand. Highly recommend."
  - "Alison|Parent of A-Level Student|My son has been receiving tutor lessons in A Level History and A Level Sociology since the beginning of this year. The tutor has been such a tremendous support, teaching him content as well as exam technique."
  - "Binh|A-Level Student|My sessions were super simple compared to what I'd experienced with other teachers. I really enjoyed my time and would highly recommend it to people."
  - "Kachi|University Student|My tutor transformed my performance in mathematical economics modules, taking me from a third class to a first. His patience and ability to adapt to my needs exceeded my expectations."
  - "Dan|A-Level Student|Really helped me get through my A-levels and made a massive difference when helping me get into my first choice of university."
  - "Mahir|A-Level Student|Harry was an excellent teacher that helped me gain real grip of the complicated areas in my subjects like economics - I'd highly recommend for anyone looking to consistently perform at their best."
  - "Alex|University Student|Can’t recommend The Degree Gap highly enough. Quick and helpful responses from Harry. He immediately matched me with a very competent tutor, who was a massive help in helping me exceed my target grade and achieve a first class degree at university."
  - "Sarom|Parent of A-Level Student|Harry has been very helpful in tutoring my son for his economics A Level. He has lots of patience, keeps lessons interesting and is a good motivator. Needless to say his subject matter is expertise is excellent."
  - "J.O.|A-Level Student|At first I was hesitant on getting a tutor, but this wasn't the case. My tutor helped me massively with my essays, topics I didn't understand, and overall confidence with the subject. I highly recommend."
  - "Charlotte|Student|I cannot recommend Harry enough. He was helpful welcoming and helped me achieve a grade higher than I was expected!"
  - "Kasia|Parent|Our tutor is a great teacher. Very helpful and knowledgeable. His experience and expertise helped my son make decisions about choosing A-level subjects and his university course. I highly recommend The Degree Gap."
  - "Leo|Student|The time I have spent with my tutor has been really valuable. He has helped with a range of topics across economics and maths, explains things really clearly, and makes the sessions enjoyable."
  - "Tommy|University Student|I wish I had found The Degree Gap when I was sitting my school exams. They helped with my dissertation work and without their help I am not sure I would have graduated so successfully and landed a successful job."

faq_1_q: "She did well at GCSE but A-Level has thrown her. Is that normal, and what does a tutor do?"
faq_1_a: "Very normal. A-Level isn't GCSE with harder numbers, it's a step up in depth and independent study, and plenty of strong GCSE students feel it in the first term. The tutor works out where the depth stopped landing, then rebuilds from there, usually on the evaluation and the multi-step questions where the marks actually sit."
faq_2_q: "Should we start A-Level tutoring in Year 12, or wait until Year 13?"
faq_2_a: "Year 12 is the stronger starting point, because it builds the foundation before the mocks that set UCAS predictions arrive. That said, plenty of Year 13 students see real gains, especially on exam technique and the long-answer questions where the top grades are won. Starting later isn't starting too late."
faq_3_q: "Can an A-Level tutor help with a degree apprenticeship application near Wigan?"
faq_3_a: "Yes. Schemes with employers like BAE Systems and the University of Greater Manchester screen on predicted grades and UCAS points, so the work that lifts a predicted grade helps directly. We can also talk through subject strategy and the application itself, drawing on the founders' own experience."
faq_4_q: "What does A-Level tutoring in Wigan cost?"
faq_4_a: "Sessions start from £37 an hour, with higher rates for the most experienced tutors and qualified examiners. The rate is agreed before anything is booked, so there are no surprises."

sitemap:
  priority: 0.7
  changefreq: monthly
---
