---
title: "Swindon GCSE Science Tutors | Online | The Degree Gap"
nav_title: "Science"
description: "Online GCSE Science tutors covering Swindon for triple and combined award. Six-mark answers, practicals and the three sciences apart, from £37 an hour."
layout: "level-subject"
location: "Swindon"
level: "GCSE"
subject: "Science"

hero_h1: "Online GCSE Science Tutors Supporting Swindon Families"
hero_lead: "Science homework that is all reading produces students who recognise the content and cannot answer the question. Our Swindon GCSE Science tutors work online on what happens between sessions as much as during them."

angle_eyebrow: "WHAT HAPPENS BETWEEN THE LESSONS"
angle_heading: "Why the Week Between Sessions Decides How Much an Hour Is Worth"
angle_body_1: "An hour a week is not very much time, and what a student does with the other hundred and sixty seven decides how far it goes. In Science the difference is stark, because the two obvious ways to revise produce completely different results. Reading over notes and watching videos feels like work and builds recognition, which is the sense that you knew something once you see the answer. Answering exam questions cold builds recall and application, which is what the paper asks for. A student can do the first for months and improve almost nothing, which is why effort and grades sometimes seem unrelated."
angle_body_2: "So the work set between sessions is deliberate rather than an afterthought: a small number of exam questions on the topic just taught, done without notes, then marked against the real scheme. Twenty to thirty minutes done that way beats two hours of reading. The first session establishes the pattern and shows your child the difference on their own answers, which is more persuasive than being told. New College Swindon, Swindon College and the school sixth forms each publish their own Science requirements, sometimes worded differently for triple and combined, so it is worth reading the current one."
angle_image: "/images/fluorescent-cells-under-microscope.jpg"
angle_image_alt: "A GCSE Science student working through a required practical question with an online tutor"
angle_stat_from: "4"
angle_stat_to: "6"
angle_stat_detail: "The kind of movement a term of weekly one-to-one Science hours usually targets once the work between sessions is exam questions."

schools:
  - "The Ridgeway School & Sixth Form College"
  - "Commonweal School"
  - "Nova Hreod Academy"
  - "St Joseph's Catholic College"
  - "Isambard Community School"
  - "Swindon Academy"

steps_heading: "Start online GCSE Science tutoring in Swindon in 3 steps"
steps_lead: "Profiles inside a day. How your child currently revises Science usually explains most of the gap on its own."
steps:
  - title: "Share your goals"
    body: "Year group and school, then the board and whether your child is on triple or combined. Tell us how your child revises Science at the moment, honestly, because that answer often explains the grade. Which of the three feels worst is the other thing worth knowing."
  - title: "Meet your match"
    body: "Two or three suggestions land within twenty four hours, narrowed by specification and by which of the three is behind rather than on Science in general. A free 15-minute video introduction comes before the first paid hour."
  - title: "Start the lessons"
    body: "Weekly one-to-one hours over Lessonspace, our shared interactive whiteboard. Exam questions from the start, marked against the real scheme in front of your child, with method written out on screen. Work between sessions is set at the end so it follows what was taught. Sessions are recorded. From £37 an hour, pause whenever you need to."

faqs:
  - q: "How does GCSE Science performance in Swindon compare to the national average?"
    a: "Swindon sits a little below the national picture on headline measures, with a wide spread between schools across the town, which matters far more to one family than the average does. Two students a few miles apart can be on different Science routes with different amounts of practical time behind them. The comparison worth making is your child's most recent paper against the paper they will sit, which is what the first session does. Unlike a published figure, that gives you something you can act on between now and the exams."
  - q: "What happens in a typical GCSE Science tutoring session?"
    a: "An hour in three parts. The first fifteen minutes go on the questions set last week, marked properly so misconceptions come to the surface instead of staying hidden. The middle half hour is the teaching, on whichever topic or skill the diagnostic pointed at, with method written out on the shared whiteboard rather than described. The last fifteen minutes are fresh exam questions on that topic, marked against the real scheme while your child watches. The next week's work is set at the end so it follows on."
  - q: "How much Science homework should my child do between tutoring sessions?"
    a: "Twenty to thirty minutes, and it should be exam questions rather than reading. Answering a question cold and then marking it against the scheme builds the thing the paper tests; re-reading notes builds the feeling of knowing, which is not the same and does not transfer. Where required practicals are the gap, short recall work on variables, apparatus and sources of error a few times a week does more than one long session. Most tutors set the work at the end of the hour so it lines up with what was just taught."
  - q: "Can a Science tutor help a child with dyslexia, ADHD or processing differences?"
    a: "Yes. The adjustments that work in Science tend to be concrete rather than general: writing calculations in a fixed layout every time so nothing is lost mid-question, learning practicals as a numbered list of variables and steps instead of a paragraph, and using the whiteboard to show a process as a diagram rather than a block of text. Several tutors do this routinely. The most useful thing you can do is tell us on the call what already works at school, including any access arrangements, rather than leaving us to discover it."
  - q: "Can I sit in on my child's Science sessions?"
    a: "You can, and plenty of parents do for the first one to see how it works. Most students settle better without an audience though, so after that we usually suggest stepping back. What replaces it is more useful anyway: every session is recorded, so you can watch any part of it later, and the tutor sends a short note on what was covered and what was set for the week. If you want a proper conversation about progress, ask for one at any point rather than waiting."
# FAQ picks: G02, A08, A09, E03, C06

reviews:
  - "Dan|A-Level Student|Really helped me get through my A-levels and made a massive difference when helping me get into my first choice of university."
  - "Marfa|Student|very engaging lessons that helped me improve my grade in maths and successfully get admitted to university"
  - "Will|University Student|Harry helped me with writing a personal statement for a masters degree. The advice he offered helped significantly, giving great insight. I would definitely recommend him to others."
  - "Lucas|A-Level Student|I was having trouble with some areas of my economics a level course before getting in contact with Harry. He walked me through everything I needed help with, from content I was weak with to exam technique that I was lacking. His insight, examples and willingness to go the extra step really helped me to regain my confidence in economics, having achieved a predicted a* with his help."
  - "Nick|Parent of A-Level Student|Sandra Sabu has been tutoring our son for Psychology A Level. Sandra has provided a solid base of tutoring outside of school lessons and helped him to improve his latest grades."
---
