---
title: "Plymouth GCSE Maths Tutors | The Degree Gap"
nav_title: "Maths"
description: "Online GCSE Maths tutors covering Plymouth, from the non-calculator paper to the grades Devonport apprenticeships read. One-to-one from £37 an hour."
layout: "level-subject"
location: "Plymouth"
level: "GCSE"
subject: "Maths"

hero_h1: "Online GCSE Maths tutoring for Plymouth: marks lost on the non-calculator paper"
hero_lead: "Paper 1 comes without a calculator, and that is where most of the quiet damage gets done. Our Plymouth GCSE Maths tutors find the arithmetic and algebra underneath the lost marks and drill it, one-to-one online over Lessonspace."

angle_eyebrow: "THE PAPER WITHOUT THE CALCULATOR"
angle_heading: "Where Plymouth Students Lose Maths Marks Before They Reach the Calculator"
angle_body_1: "Maths at GCSE is two separate skills wearing one grade. There is the fluency Paper 1 wants, fractions, indices and algebra done by hand with nothing on the desk to check the arithmetic. Then there is the problem solving at the back of Papers 2 and 3, where a question describes a situation and never names the topic. Plymouth students turn up with either gap, sometimes both. The culprits are narrow and they repeat: rearranging a formula with the unknown in the wrong place, ratio inside a wordy problem, percentage change run backwards, and multi-mark questions where partial working was worth marks nobody claimed."
angle_body_2: "So the first hour is a recent paper marked question by question, with the lost marks sorted into fluency, method and reading. After that the working gets written out on screen rather than talked through, because a student who watches a method does not own it. And this reaches past the Maths grade. Sixth forms at Devonport High School for Boys and Plymouth High School for Girls publish their own requirements, usually an average across the best GCSEs plus a grade in the subject itself, while the engineering intakes at Devonport and Princess Yachts name Maths on the application form. One grade, two doors."
angle_image: "/images/maths-equations-close-up.jpg"
angle_image_alt: "A Plymouth GCSE Maths student working through a non-calculator question on a shared whiteboard"
angle_stat_from: "4"
angle_stat_to: "6"
angle_stat_detail: "Where a term of weekly one-to-one GCSE Maths hours is typically pointed once Paper 1 stops leaking marks."

schools:
  - "Devonport High School for Boys"
  - "Devonport High School for Girls"
  - "Plymouth High School for Girls"
  - "Hele's School"
  - "Plymstock School"
  - "UTC Plymouth"

steps_heading: "Start online GCSE Maths tutoring in Plymouth in 3 steps"
steps_lead: "Two or three profiles inside a day, and timing matters here, because sixth form and college applications go in during the autumn of Year 11."
steps:
  - title: "Share your goals"
    body: "Year group, school, exam board and tier, plus the grade now and the grade the next step asks for. Tell us whether the damage is on the non-calculator paper or on the wordy problems at the back, if you know. If you do not, a marked mock will tell us."
  - title: "Meet your match"
    body: "Two or three profiles land the next day, chosen on the specification, the tier and the kind of gap the marked paper points at. There is a free 15-minute video meeting with the tutor before anything is paid for."
  - title: "Start the lessons"
    body: "A weekly hour on Lessonspace, our shared whiteboard, with every step of the working written out on screen instead of done in somebody's head. Sessions can go to twice a week in the run-up to mocks. From £37 an hour, paid one session at a time."

faqs:
  - q: "Can a Plymouth GCSE Maths tutor help my child apply for a Babcock apprenticeship at Devonport?"
    a: "Yes, and the grade work comes first. Babcock is taking on well over two hundred apprentices at Devonport for the 2026 intake, and its engineering trades routes publish their own entry requirements, generally around five GCSEs at grade 4 including Maths, English and a science or technology subject. Those requirements move between intakes, so work from this year's rather than from what an older sibling remembers. The tutoring aims at the Maths grade the application rests on, and at the fluency the assessment stage afterwards asks for."
  - q: "Does it matter whether my child sits AQA, Edexcel or OCR Maths?"
    a: "It matters at the margins, and the margins are where grades move. The content overlaps almost entirely. What differs is the wording of the longer questions, how much of a multi-step problem comes scaffolded, and which formulae sit on the sheet rather than in your child's head. A tutor who knows the specification stops guessing in session one. We confirm the board on the consultation call before anybody is matched, because it decides which past papers the whole plan is built on."
  - q: "Should my child focus on the calculator or the non-calculator paper?"
    a: "Both count the same, so the real question is where the marks are going. Paper 1 rewards fluency: fractions, indices, standard form, algebra done by hand with no screen to check it against. Papers 2 and 3 reward accuracy across several steps and the reading of a wordy problem. Most students leak marks in one of those two ways rather than in both. Ten minutes with a marked paper tells you which, and the term then goes there rather than over everything."
  - q: "What credentials does The Degree Gap have as a tutoring agency?"
    a: "We were voted GCSE Tutoring Specialists of the Year at the Prestige Awards in 2025 and again in 2026, and there are more than a hundred five-star reviews on Google. Harry and Joe interview every tutor themselves before anyone joins the platform, and Harry read Economics at the LSE. We work directly with 15 or more UK secondary schools and run parent webinars through the year. None of that teaches your child ratio. It does mean somebody has checked the person who will."
  - q: "Can my child have Maths sessions during the school holidays?"
    a: "Yes, and half term and Easter are often the most productive weeks of the year in Maths, because a long question can be worked through without a homework deadline sitting behind it. Plenty of Plymouth families keep the weekly hour going through the holidays and add a second one in the fortnight before mocks. Others stop for August. Sessions are booked one at a time, so it stays a conversation with the tutor rather than a plan committed to in September."
# FAQ picks: A02, A11, C05, E04, G04

reviews:
  - "Ellen|Parent of GCSE Student|My daughter was struggling with confidence in Maths GCSE and we decided to get a tutor. After lots of recommendations we booked a chat with Harry who listened to my daughters needs and gave a selection of tutors to choose from. He made it clear that if it didn’t feel like a good match we could try other tutors. We opted for Avery and she was a complete star, listened, made it simple, set good homework and gave my daughter her confidence back in her abilities and gave her the confidence to face Maths GCSE exams. Highly recommend Degree Gap and will be back again in September with my youngest embarking on her GCSEs"
  - "Jessica|Student|I highly recommend Harry – he was excellent in helping me with both Maths and English for my GCSEs. Thanks to his support, I passed both subjects with great results, which gave me the foundation and confidence to go on to succeed in my A-levels and complete my degree!!"
  - "Marfa|Student|very engaging lessons that helped me improve my grade in maths and successfully get admitted to university"
  - "Keira|GCSE Student|Has definitely helped me increase my grades from an E to a B!!"
  - "Dawn|Parent|The Degree Gap took time to understand my child’s worries. They paired me with Connor, who my daughter got on well with. Her confidence has skyrocketed since. Thanks again."

sitemap:
  priority: 0.7
  changefreq: monthly
---
