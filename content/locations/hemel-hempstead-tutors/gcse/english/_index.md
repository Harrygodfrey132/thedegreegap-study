---
title: "Hemel Hempstead GCSE English Tutors | Online | The Degree Gap"
nav_title: "English"
description: "Online GCSE English tutors covering Hemel Hempstead. Work on the writing section marks, spelling, punctuation and structure, from £37 an hour."
layout: "level-subject"
location: "Hemel Hempstead"
level: "GCSE"
subject: "English"

hero_h1: "Online GCSE English Tutoring for Hemel Hempstead Students"
hero_lead: "Half the English Language paper is your child's own writing, marked on how it is built and how accurately it is written. Our Hemel Hempstead GCSE English tutors work online on the half that is easiest to move."

angle_eyebrow: "THE HALF THAT IS THEIR OWN WRITING"
angle_heading: "Why the Writing Section Is the Most Winnable Part of English"
angle_body_1: "Students and parents both tend to think of English as being about texts, and then half of the Language paper turns out to be a piece of the student's own writing, marked on structure, vocabulary, sentence variety and technical accuracy. It is worth a large block of marks and it is the most predictable part of the whole subject, because nothing in it is unseen. Yet it is usually the least practised, partly because it feels like the bit you cannot revise for and partly because it gets treated as a test of imagination, which it is not."
angle_body_2: "It is a test of craft, and craft can be taught. A structure that works whatever the prompt turns out to be, deliberate variation in sentence length, a small stock of precise vocabulary used accurately rather than a large one used hopefully, and paragraphing that signals shape. Add careful attention to the technical accuracy marks, which students give away in numbers. The first hour reads a recent piece against the actual criteria. Hemel Hempstead students go on to sixth forms locally and towards St Albans, and to West Herts College."
angle_image: "/images/handwriting-study-notes-with-highlighters.jpg"
angle_image_alt: "A GCSE English student planning an essay answer with an online tutor"
angle_stat_from: "4"
angle_stat_to: "6"
angle_stat_detail: "Where a term of weekly one-to-one GCSE English sessions is typically aimed once the writing section is being worked on directly."

schools:
  - "Cavendish School"
  - "Hemel Hempstead School"
  - "The Astley Cooper School"
  - "Adeyfield Academy"
  - "Longdean School"
  - "John F Kennedy Catholic School"

steps_heading: "Start online GCSE English tutoring in Hemel Hempstead in 3 steps"
steps_lead: "Profiles inside a day. Send a piece of your child's own writing, not just an essay about a text."
steps:
  - title: "Share your goals"
    body: "Year group, school, exam board, and whether the worry is the reading side, the writing side or both. Name the set texts for Literature. A piece of your child's own timed writing with marks on it is the most useful thing you can send."
  - title: "Meet your match"
    body: "Profiles the next day, matched on the board and the set texts, and on experience with the writing paper, which not every English tutor works on directly. Your child meets the tutor on a free 15-minute video call first."
  - title: "Start the lessons"
    body: "Weekly hour-long sessions on Lessonspace, where tutor and student write on the same screen. Writing gets planned, produced to time and marked against the real criteria on screen, including the technical accuracy marks most students give away. Sessions are recorded. From £37 an hour, with nothing to sign."

faqs:
  - q: "Is the GCSE English standard in Hemel Hempstead similar to nearby areas?"
    a: "Comparing towns is less useful than it sounds, because your child sits in one school following one specification with one set of texts, and the sixth form they apply to publishes its own English requirement that changes between years. Hertfordshire sits above the national picture on headline measures, with a wide spread inside it. The honest answer to how your child is doing is in their most recent marked script and in the requirement published by the specific place they want, not in an area figure."
  - q: "What is the difference between AQA, Edexcel and OCR GCSE English, and does it matter for tutoring?"
    a: "It matters a great deal. The boards set different texts, weight context differently, and structure the Language papers differently, including how the writing task is framed and how many marks go to technical accuracy. A student prepared for one board's paper can be thrown by another's format with identical knowledge. This is why the board and the set texts are confirmed before anyone is proposed, and why a tutor who has taught your child's exact combination is worth waiting an extra day for."
  - q: "Why is my child struggling with GCSE English?"
    a: "Often because the writing has never been taught as a skill in its own right, separate from the reading. Schools cover the texts carefully and then set work that has its own rules, and the space between understanding a book and scoring on a question about it is where marks disappear. On the Language paper, students lose a surprising number of marks on technical accuracy and on structure, both of which are learnable and neither of which feels like it should matter as much as it does."
  - q: "How do you vet your GCSE English tutors?"
    a: "About three in every hundred applicants come through. There is an application form, a subject knowledge check and a live interview, and the interview is deliberately uncomfortable, because teaching one-to-one is uncomfortable when a student is stuck and the clock is going. Someone who can stay clear in that moment is worth a great deal. Harry and Joe interview every one of them, which is slower than delegating it and produces better matches."
  - q: "What if my child does not want a tutor?"
    a: "It is the worry parents raise most, and forcing it rarely works. Most resistance happens before a student has met anyone and is about what having a tutor seems to say about them rather than about the hour. The free 15-minute video meeting exists for that: your child meets the tutor first and gets a say in the match. Most students who were against the idea have come round by the second or third session. If they genuinely still do not want it after that, pause it."
# FAQ picks: G06, A02, A07, E01, C07

reviews:
  - "Jeanne|Student|I can’t recommend The degree gap enough! Harry provided quick and extremely helpful feedback on my personal statement for my master’s applications. Thank you, Harry!"
  - "Darren|Parent|A really helpful experience for supporting our Son.Thank you Joe,Harry and Matt."
  - "Joel|Student|The degree gap is an amazing place and have helped me achieve so much and I am looking forward to it helping it achieve a higher class of skills and ready for the RAF"
  - "joe|Student|Highly recommend the Degree Gap as a tutoring agency! Their platform is easy to use and having used tutors across a variety of subjects, they have all been consistently excellent."
  - "Kasia|Parent|Harry is a great tutor. He's very helpful and knowledgeable. He's git goid understanding of student's needs as well as current situation when it comes to university education in the UK. His experience and expertise helped my son make some decisions about choosing A level subject and university course. I highly recommend Harry and Degree Gap."
---
