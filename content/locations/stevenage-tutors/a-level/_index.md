---
title: "Stevenage A-Level Tutors | Year 12 and Year 13 | The Degree Gap"
description: "Stevenage A-Level tutoring for Year 12 and Year 13, matched by specification. Support for the GSK, Airbus and MBDA apprenticeship routes as well as UCAS."
layout: "subject"
location: "Stevenage"
level: "A-Level"

hero_heading_line1: "Stevenage A-Level Tutors for the Two Years That Decide the Offer"
hero_heading_line2: ""
hero_lead: "Not every Stevenage student is aiming at university, and the town's biggest employers ask for grades too. Our Stevenage A-Level tutors work to whichever gate a student is actually walking through, whether that is a UCAS offer or a degree apprenticeship at GSK or Airbus."

grade_from: "D"
grade_to: "B"
grade_stat: "The kind of two-band move weekly one-to-one work across Year 12 and Year 13 is built to deliver."

first_lesson_eyebrow: "WHERE YEAR 12 GOES WRONG"
first_lesson_heading: "The Jump From GCSE That Nobody Warns Students About"
first_lesson_context: "A student can leave Year 11 with a good set of grades and be genuinely lost by the second half of Year 12. That is not a failure of effort or of the school, it is that A-Level asks for something GCSE did not: sustained argument, method held across several steps, and independent practice in a volume nobody has ever required before. Students who could carry GCSE on recall find that recall alone now scores about half. The fix is a change in how they work rather than more hours of the same, which is why the first session looks at how a paper was attempted rather than at what was known."

first_lesson_quote: "The A-Level tutoring made such a difference to my son, who had left studying until the final hour, managing to turn E and U grades into 3 C grades. Amazing."
first_lesson_quote_name: "Joanna"
first_lesson_quote_role: "Parent of A-Level Student"
first_lesson_quote_grade: "A-Level"

tutor_strip_heading: "Stevenage A-Level tutors matched by specification and by module"
tutor_strip_body: "At A-Level the module choices matter as much as the subject. Two students both doing Physics can be sitting different option papers, and a tutor matched on the subject name alone will spend the first fortnight catching up. We confirm the board and the modules on the consultation call before suggesting anyone. Browse profiles, or let us match your child."

pathways_heading: "What Comes After A-Levels in Stevenage"
pathways_lead: "Three routes, and the grades each tends to ask for."
pathways:
  - title: "Universities"
    body: >
      Hertfordshire, Cambridge, Nottingham, Loughborough and the London universities all take students from north Hertfordshire each year, with the commute into London making a wider set of options realistic than the map suggests. Offers hang on the predicted grade first, which is set well before any real paper is sat.
  - title: "Degree Apprenticeships"
    body: >
      This is where Stevenage is genuinely unusual. GSK's research campus, Airbus Defence and Space, MBDA and the wider cell and gene therapy cluster all recruit locally into degree apprenticeships. These are competitive, they publish their own entry requirements which move year to year, and Maths carries most of the weight on the engineering and science routes.
  - title: "Career Pathways"
    body: >
      Some students go directly into technician and laboratory roles across the same employers, or into construction and logistics work along the A1 corridor. A gap year used to strengthen a reapplication is common too, especially where a first attempt at a competitive course fell one grade short.

reviews:
  - "Joanna|Parent of A-Level Student|The A level tutoring made such a difference to my son, who had left studying until the final hour, managing to turn E and U grades into 3 C grades. Amazing"
  - "Daljit|Parent of A-Level Student|My son gained almost two grades higher than jis year 13 mock paper. His tutor Jacob was excellent"
  - "Alexander|Parent|Degree Gap helped my kid go from Cs to As. They have a great selection of tutors who not only assist with the curriculum but also helped with university choices and application."
  - "Nick|Parent of A-Level Student|Sandra Sabu has been tutoring our son for Psychology A Level. Sandra has provided a solid base of tutoring outside of school lessons and helped him to improve his latest grades."
  - "Alison|Parent of A-Level Student|My son has been receiving lessons with Jacob Berry in A-Level History and subsequently A-Level Sociology since the beginning of this year. Jacob has been such a tremendous support for my son, teaching him content as well as exam technique."
  - "Sureet|Parent of A-Level Student|Haneet has really helped my son in navigating his A level Physics all whilst being kind and supportive."
  - "Dan|A-Level Student|Really helped me get through my A-levels and made a massive difference when helping me get into my first choice of university."
  - "Cat|Parent of A-Level Student|My 18 year old son had A-level Economics tutoring with Harry.  He was excellent, knew exactly what he needed to learn and was able to explain in terms which made it simple to understand.  The online platform was convenient and easy to use."
  - "Theresa|Parent|Harry and Joe were extremely helpful with a wealth of knowledge and contacts that helped my son to follow the right path."
  - "Lisa|Parent of Two Students|Joe and Harry at The Degree Gap have exceeded all my expectations.  They have gone above and beyond to pair both my sons with the appropriate tutors, all of whom have been exceptional.  Both my sons exceeded well above their expected grades in their GCSE's and A Levels and got into their first choice Universities. I cannot recommend them highly enough."
  - "Marfa|University Student|very engaging lessons that helped me improve my grade in maths and successfully get admitted to university"
  - "Nicolai|University Student|Had troubles connecting with tutors until I met Harry, helped me understand the curriculum and theories in a way that stuck with me, also made each session feel calm and relaxing. Highly recommend"

faq_1_q: "Can a tutor help with a GSK or Airbus degree apprenticeship application?"
faq_1_a: "Yes, and Maths is usually the grade that decides eligibility on the technical routes. The Stevenage employers publish their own entry requirements, those vary by scheme and shift between years, so work from the current listing rather than from what someone was told last year. Where a student sits just under the line, that is a short and specific piece of work rather than a full programme, and the application itself often carries an essay or a test worth preparing for."
faq_2_q: "My son is predicted a D and needs a B. Is that realistic?"
faq_2_a: "It depends entirely on where the marks are going, and that is answerable from a paper rather than from the prediction. If the content is broadly there and the losses are in method, timing and unfinished questions, two bands across a year is a normal outcome. If large parts of the specification were never secure, the honest answer is that it will take longer than one term and may mean a narrower target. We would rather say that on the call than take the booking and disappoint you in March."
faq_3_q: "How much does A-Level tutoring cost in Stevenage?"
faq_3_a: "From £37 an hour, agreed before any session is booked. The rate rises with the tutor's experience, so a recent graduate teaching their own subject sits near the entry rate and a qualified teacher or examiner sits higher. The hour covers the lesson, the preparation, any papers set and the feedback. No joining fee and no contract, and as an agency we retain a margin on the rate, which covers the matching and the support around it."
faq_4_q: "Can we do fewer sessions to keep the cost down?"
faq_4_a: "Yes, and it is often the sensible shape. One hour a week through Year 12, with a second added only for the eight weeks before mocks, costs far less than two hours held all year and tends to work at least as well. The other option is a short block of six to eight sessions aimed at one thing, usually essay structure or a single module that is dragging the grade. Say what the budget is on the call and we will build to it rather than around it."

sitemap:
  priority: 0.7
  changefreq: monthly
---
