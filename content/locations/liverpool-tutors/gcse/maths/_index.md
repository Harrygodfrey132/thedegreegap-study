---
title: "Online GCSE Maths Tutors | Liverpool | The Degree Gap"
nav_title: "Maths"
description: "Liverpool GCSE Maths tutors matched to the AQA, Edexcel or OCR paper your child actually sits. One-to-one work on higher tier and Paper 3, from £37 an hour."
layout: "level-subject"
location: "Liverpool"
level: "GCSE"
subject: "Maths"

hero_h1: "Online GCSE Maths Tutors Covering Liverpool"
hero_lead: "The revision is happening. The mark isn't moving. Our Liverpool GCSE Maths tutors start by marking a recent higher paper, then work out whether the marks are going on method or on the way the question was read."

angle_eyebrow: "WHY GCSE MATHS IN LIVERPOOL, SPECIFICALLY"
angle_heading: "Where a GCSE Maths Tutor Finds the Marks That Move a Liverpool Grade"
angle_body_1: "Liverpool asks two different things of a GCSE Maths tutor. One student needs a secure grade 5 to hold a college place. Another is a few marks short of what St Edward's or Blue Coat want before A-Level Maths. Both tend to lose marks in the same four places: algebraic fractions buried inside a multi-mark question, circle theorem proofs that need a reason written on every line, iterative numerical methods, and the long wordy problems at the back of Paper 3. Very little of that is missing knowledge. It's question style."
angle_body_2: "So the first lesson is a marked paper, not a lesson plan. The tutor separates marks lost to method from marks lost to misreading what was asked, because those need different fixes. From there the plan targets whichever block is actually bleeding, whether that means going back to Year 9 algebra or drilling the last six questions of Paper 3 until timing stops being the problem. For a Cardinal Heenan or St Edward's sixth-form place, or the numeracy stage of a Jaguar Land Rover apprenticeship at Halewood, that gap is usually what the offer turns on."
angle_image: "/images/maths-equations-close-up.jpg"
angle_image_alt: "A GCSE Maths student working through a higher-tier algebra question with a tutor"
angle_stat_from: "5"
angle_stat_to: "7"
angle_stat_detail: "The lift we most often work towards across a term of weekly one-to-one GCSE Maths sessions."

schools:
  - "Liverpool Blue Coat School"
  - "St Edward's College"
  - "Calderstones School"
  - "Cardinal Heenan Catholic High School"
  - "Gateacre School"
  - "The Alsop High School"
  - "Archbishop Beck Catholic College"

steps_heading: "Start GCSE Maths tutoring covering Liverpool in 3 steps"
steps_lead: "Most Liverpool families are matched inside 24 hours, which matters most between September and the March mocks that set sixth-form predicted grades."
steps:
  - title: "Share your goals"
    body: "Tell us the exam board, the tier and the target grade. Liverpool schools run AQA, Edexcel and OCR between them and the papers are not interchangeable. We also ask which Year 9 or Year 10 topics have already been flagged, because that usually points at where the diagnostic should start."
  - title: "Meet your match"
    body: "Two or three tutor profiles arrive within 24 hours, with a free 15-minute meeting before anything is booked. We match on the specification and on the part of the paper costing marks, so a student losing them on Paper 3 problem solving gets a tutor who teaches that end of the paper."
  - title: "Start the lessons"
    body: "Weekly hour-long sessions on Lessonspace, our shared whiteboard, so every worked solution is saved and can be revisited. Many Liverpool Year 11s add a second weekly hour in the run-up to the November and March mocks. From £37 an hour."

faqs:
  - q: "Can a Liverpool GCSE Maths tutor help my child apply for a Jaguar Land Rover apprenticeship at Halewood?"
    a: "Yes, and Maths is usually the subject that decides it. Engineering apprenticeship schemes around Merseyside, including the Halewood plant and the Speke sites, publish their own requirements and these move year to year, but grade 4 to 5 in Maths and English is the usual floor. Most also run a numeracy assessment before interview. A tutor spends that preparation time on the algebra, ratio and multi-step problem solving those assessments come back to rather than on the whole syllabus."
  - q: "Should my child focus on the calculator or the non-calculator paper?"
    a: "Both, because they carry equal weight, so the real question is which one is losing more marks. The non-calculator paper rewards arithmetic fluency and clean algebraic manipulation. The calculator papers reward accuracy across multi-step work and the patience to read a long question properly. Most students leak marks in different places on each. A tutor's first diagnostic is often just marking one of each and seeing which pattern looks worse."
  - q: "Should my child sit foundation or higher tier GCSE Maths?"
    a: "Higher tier runs from grade 4 up to 9, foundation caps at grade 5. If your child is sitting around a borderline 5 in Year 10 mocks, higher tier is usually the right call but it needs targeted preparation, because the back third of the paper is where those students lose ground. At a 3 or a low 4, foundation lowers the risk of walking out with very little. It's a decision worth making with a marked paper in front of you rather than on a feeling."
  - q: "How do you vet your GCSE Maths tutors?"
    a: "Every tutor clears a founder-led interview that roughly 3% of applicants pass. Joe and Harry run them personally. We test subject knowledge, then how clearly someone explains a method they know cold to a student who does not, which is the part most applicants come unstuck on. All our tutors are graduates of top Russell Group universities and most have at least two years of one-to-one experience. A degree on its own has never been enough."
  - q: "Is GCSE Maths tutoring actually worth the money?"
    a: "It depends on the size of the gap, and we would rather say so on the call than after you've paid. For a student sitting below the grade a sixth form or a college course asks for, a one or two band lift opens a door that stays shut otherwise, and that is worth the money. For a student already comfortable at their target, a short block on exam technique usually does more than a year of weekly sessions. Sometimes the honest answer on the consultation call is that tutoring isn't what your child needs right now."

reviews:
  - "Ellen|Parent of GCSE Student|My daughter was struggling with confidence in Maths GCSE and we decided to get a tutor. We were given a selection of tutors to choose from and told that if it did not feel like a good match we could try others. Our tutor was a complete star, listened, made it simple, set good homework and gave my daughter her confidence back. Highly recommend."
  - "Keira|GCSE Student|Has definitely helped me increase my grades from an E to a B."
  - "Jessica|GCSE Student|I was helped with both Maths and English for my GCSEs and passed both subjects with great results, which gave me the foundation and confidence to go on to succeed in my A-levels and complete my degree."
  - "Chris|Parent of GCSE Student|Very good tutoring for my daughter and good communication with parents. My daughter passed both English and Maths GCSE."
  - "Philippa|Parent of GCSE Student|I was really pleased to discover the Degree Gap when my daughter needed help getting the grades she needed for GCSEs. The team offer a really personalised service and I felt really well looked after from the start. There are lots of tutors to choose from and all subjects are catered for, with great flexibility on timings. Highly recommend."

sitemap:
  priority: 0.6
  changefreq: monthly

# FAQ picks: G04, A11, A12, E01, C03
---
