---
title: "Peterborough GCSE Tutors | Expert Tutors | The Degree Gap"
description: "Peterborough GCSE tutors who rebuild the gap before mocks turn it into a predicted grade. One-to-one tutoring matched on exam board, from £37 an hour."
layout: "subject"
location: "Peterborough"
level: "GCSE"

hero_heading_line1: "Peterborough GCSE Tutors Who Rebuild Before the Mocks, Not After"
hero_heading_line2: ""
hero_lead: "Mocks do not create the problem, they publish it. Our Peterborough GCSE tutors would rather find the gap in October than argue with a predicted grade in March, so the first session is a diagnostic and the plan follows from it."

grade_from: "E"
grade_to: "B"
grade_stat: "A genuine climb from an E to a B, in the student's own words, after regular one-to-one sessions."

first_lesson_eyebrow: "AN E TO A B, HER WORDS"
first_lesson_heading: "The First Session With a Peterborough GCSE Tutor"
first_lesson_context: "Attainment across Peterborough sits below the national picture, though that tells you very little about any one student. What the average hides is how ordinary the causes usually are. A term of supply teaching, a topic missed through illness, a method that was never quite understood and got built on anyway."
first_lesson_quote: "Has definitely helped me increase my grades from an E to a B."
first_lesson_quote_name: "Keira"
first_lesson_quote_role: "GCSE Student"
first_lesson_quote_grade: "Grade E to Grade B"

tutor_strip_heading: "Peterborough GCSE tutors who fix foundations, not just revise"
tutor_strip_body: "Students at Jack Hunt, Thomas Deacon Academy and Hampton College sit different specifications, so we match on the paper before anything else. Each tutor clears a founder-led interview that only about 3% of applicants pass. Browse the profiles, or let us make the introduction."

pathways_heading: "Post-16 Routes For Peterborough Year 11s"
pathways_lead: "Wondering what comes next? These are the three routes local families consider most."
pathways:
  - title: "Sixth Forms"
    body: >
      Peterborough College takes the bulk of the city's post-16 students, with sixth forms at Thomas Deacon Academy and The King's School running alongside it. Each publishes its own entry requirements, and A-Level courses generally ask for a grade in the subject a student wants to carry on.
  - title: "Apprenticeships"
    body: >
      Perkins Engines runs engineering apprenticeships from its Peterborough plant and is one of the employer partners behind the degree apprenticeships at ARU Peterborough. Most routes ask for grade 4 to 5 in Maths and English, with the technical ones looking at Science too.
  - title: "Further Education"
    body: >
      Peterborough College and the wider Inspire Education Group carry most of the Level 3 and T Level provision in the city. A confident grade 4 or 5 in Maths and English opens the majority of those courses.

reviews:
  - "Keira|GCSE Student|Has definitely helped me increase my grades from an E to a B!!"
  - "Augusta|Parent of GCSE Student|The support given to my son by The Degree Gap's tutors was fabulous and he passed all his GCSE subjects. They provided experienced and organised tutors. Tutors provided resources targeting specific areas of difficulty. The website is easy to use and tutors were polite and on time."
  - "Chris|Parent of GCSE Student|Very good tutoring for my daughter and good communication with parents. My daughter passed both English and Maths GCSE."
  - "Ellen|Parent of GCSE Student|My daughter was struggling with confidence in Maths GCSE and we decided to get a tutor. We were given a selection of tutors to choose from and told that if it did not feel like a good match we could try others. Our tutor was a complete star, listened, made it simple, set good homework and gave my daughter her confidence back. Highly recommend."
  - "Omo|Parent of GCSE Student|My son, who is in Year 10, really struggled with English and was completely disengaged from the subject. After six months of tutoring, he has improved from a grade 5 to a 6/7. He now looks forward to his lessons. Through one-to-one tutoring he has learned effective strategies that help him approach English in a way that resonates with him."
  - "Nazrul|Parent|My daughter found the tutors were very helpful for her GCSE thank you"
  - "Bryan|GCSE Student|Helped me a lot and really showed me what i need to improve on"
  - "Philippa|Parent of GCSE Student|I was really pleased to discover the Degree Gap when my daughter needed help getting the grades she needed for GCSEs. The team offer a really personalised service and I felt really well looked after from the start. There are lots of tutors to choose from and all subjects are catered for, with great flexibility on timings. Highly recommend."
  - "Jessica|GCSE Student|I highly recommend Harry – he was excellent in helping me with both Maths and English for my GCSEs. Thanks to his support, I passed both subjects with great results, which gave me the foundation and confidence to go on to succeed in my A-levels and complete my degree!!"
  - "Heidi|Parent|Our tutor is simply amazing with our daughter. He understands her completely and her sessions are helping her with her school work so much. We cannot thank him enough."
  - "Isabel|Parent of iGCSE Student|The Degree Gap gave our son the support he needed for his iGCSEs, especially in Maths and Economics. Lessons were clear, engaging, and built his confidence. Highly recommended."
  - "Annette|Parent|This company helped us find a tutor for Geography and my son has excelled so much in just a few months. Our tutor has gone above and beyond, demonstrating incredible patience and encouraging my son to believe in himself. He also manages neurodiversity effectively."
  - "Sorland|Grandparent|My Granddaughter uses the Degree Gap tutoring services across a few subjects. She is not only improving academically but really enjoys the setup and structure too. I would recommend to anyone."
  - "Charlotte|Student|I cannot recommend Harry enough. He was helpful welcoming and helped me achieve a grade higher than I was expected!"
  - "Theresa|Parent|The Degree Gap was fantastic. Their knowledge and guidance helped my son find direction in both his studies and future plans."
  - "Marfa|Student|very engaging lessons that helped me improve my grade in maths and successfully get admitted to university"

faq_1_q: "The mock came back worse than we expected. What happens now?"
faq_1_a: "The paper becomes the plan, which is the one good thing about a bad mock. A tutor marks it properly rather than reading the total, works out which question types lost the most, and points the next few weeks at those. Mocks in Year 11 also feed the predicted grade sixth forms see, so there is usually a reason to move quickly rather than wait for the summer."
faq_2_q: "My daughter says she is just not good at Maths. How do you get past that?"
faq_2_a: "By finding something she can do and making that visible fast. Almost nobody is bad at Maths across the board. There is usually one method underneath everything that never landed, and once it does a lot of topics stop being frightening at the same time. The belief tends to shift before the grade does, and parents notice it first in how she talks about the subject."
faq_3_q: "Do you cover the exam boards Peterborough schools use?"
faq_3_a: "Yes, and we ask about it first. Schools across the city run AQA, Edexcel and OCR between them and the papers genuinely differ, not in content so much as in how questions are worded and what the mark schemes reward. A tutor on the wrong board teaches well and still leaves marks behind, so we match on specification and tier from the start."
faq_4_q: "How quickly can we start?"
faq_4_a: "Usually inside a week. You speak to Harry or Joe first, which takes about fifteen minutes and commits you to nothing. Two or three tutor profiles follow within 24 hours, and you get a free fifteen-minute meeting with whoever you pick before any money changes hands. First paid session is normally a few days after that."

sitemap:
  priority: 0.7
  changefreq: monthly
---
