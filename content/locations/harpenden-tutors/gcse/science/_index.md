---
title: "Harpenden GCSE Science Tutors | Online | The Degree Gap"
nav_title: "Science"
description: "Online GCSE Science tutors covering Harpenden for triple and combined award. The Year 10 to Year 11 step up and the tier decision, from £37 an hour."
layout: "level-subject"
location: "Harpenden"
level: "GCSE"
subject: "Science"

hero_h1: "Online GCSE Science Tutors for Harpenden Students"
hero_lead: "Science content does not rise evenly, it steps, and the step lands in the middle of the course. Our Harpenden GCSE Science tutors work online on the topics that arrive after it."

angle_eyebrow: "THE STEP IN THE MIDDLE"
angle_heading: "Where the Science Course Gets Suddenly Harder"
angle_body_1: "GCSE Science does not get steadily harder across two years. It steps, and for most specifications the step lands somewhere around the end of Year 10 and the start of Year 11, when the topics start demanding calculation, multi-stage reasoning and proper use of the required practicals rather than description. Students who coasted comfortably through the first half on recall alone meet that step and stall, and because it coincides with mock season, it reads as a sudden decline rather than as a course that changed shape."
angle_body_2: "Knowing where the step is makes it manageable. The topics that follow it need to be learned differently, with method written out and practised rather than read, and a student who adjusts their approach at that point rarely has trouble afterwards. It is also the moment the tier entry gets reviewed, so it is worth having evidence to hand. The first hour identifies where your child sits relative to the step. Harpenden students go on to the local sixth forms, others across St Albans, and Oaklands College, whose Science requirements differ for triple and combined."
angle_image: "/images/fluorescent-cells-under-microscope.jpg"
angle_image_alt: "A GCSE Science student working through a required practical question with an online tutor"
angle_stat_from: "5"
angle_stat_to: "7"
angle_stat_detail: "Where a term of weekly one-to-one GCSE Science sessions is typically aimed once the harder half of the course is being handled properly."

schools:
  - "Sir John Lawes School"
  - "St George's School Harpenden"
  - "Roundwood Park School"
  - "Sandringham School"
  - "St Albans Girls' School"
  - "Beaumont School"

steps_heading: "Start online GCSE Science tutoring in Harpenden in 3 steps"
steps_lead: "Profiles inside a day. If Science was fine until the end of Year 10, this is almost certainly what happened."
steps:
  - title: "Share your goals"
    body: "Year group, school, exam board, triple or combined, and roughly when Science stopped feeling comfortable. If the answer is around the end of Year 10, that tells us most of what we need to plan the term."
  - title: "Meet your match"
    body: "Two or three suggestions land within twenty four hours, shortlisted on the specification, the tier and the science that needs the hours. A free quarter-hour video meeting happens before anyone commits, so your child has a say."
  - title: "Start the lessons"
    body: "A weekly hour on the shared Lessonspace whiteboard we built for this. The harder topics get taught with the method written out and practised rather than described, and required practicals broken into named parts. Sessions are recorded. From £37 an hour, and nothing commits you past the next session."

faqs:
  - q: "How does GCSE Science performance around Harpenden compare to the national average?"
    a: "Hertfordshire sits above the national picture on headline measures, and a county figure still says very little about one child on one tier following one specification. Schools around Harpenden and St Albans have different amounts of practical time and different Science routes. The comparison worth making is your child's most recent paper against the paper they are entered for, which is what the first session looks at and which gives you something to work on this term rather than a number."
  - q: "When is the best time to start GCSE Science tutoring in Year 11?"
    a: "September to February, and the earlier the better. Winter mocks feed the predicted grades sixth forms read when making conditional offers, so work landing before them counts twice. In Science there is an extra reason: the harder half of the course is being taught during exactly that window, so a tutor working alongside it stops the gap forming rather than repairing it later. Starting after mocks still helps, but becomes triage rather than teaching."
  - q: "Should my child take foundation or higher tier GCSE Science?"
    a: "It depends on where your child sits now rather than where anyone hopes they will finish. Higher tier reaches the top grades but scores very little below its floor, while foundation caps lower and makes the marks far more reachable. A student close to the boundary is often right on higher with focused work on the topics that decide it. A student well below usually does better on foundation. Since the entry is often reviewed around the middle of the course, a real paper marked honestly is worth having."
  - q: "What grade improvements have your Science tutors actually delivered?"
    a: "We do not promise grades, because nobody honestly can. What we can describe is the pattern. Where a student stalled at the step into the harder topics and the content is otherwise sound, a term of weekly hours moving a band is common, since the fix is a method change rather than a rebuild. Where the gap runs back further, the same term brings visible movement without the full band, and a good tutor says so early. The reviews here are real and unedited."
  - q: "How quickly can my child start GCSE Science tutoring in Harpenden?"
    a: "In practice, a few days. The limiting factor is almost never us, it is finding a slot in your week and your child's. The consultation call can usually happen the same day or the next; profiles follow within a day of that; the introductory meeting takes fifteen minutes and can be squeezed into most evenings. Families who tell us it is urgent, because a tier decision is pending or mocks are close, generally start faster than families who are planning ahead."
# FAQ picks: G02, A03, A12, E05, C10

reviews:
  - "Priya|Parent of GCSE Student|Thank you to Harry and Joe for a very informative webinar on a parent's guide to supporting GCSE success. Great advice and tips given and they sent useful free resources by email which my daughter will use to help with her upcoming exams."
  - "Veronica|Parent|Very useful information and tips!! For parents as it was mentioned in the session it would be good to provide one for the students as general guidance !!"
  - "Ellen|Parent of GCSE Student|My daughter was struggling with confidence in Maths GCSE and we decided to get a tutor. After lots of recommendations we booked a chat with Harry who listened to my daughters needs and gave a selection of tutors to choose from. He made it clear that if it didn’t feel like a good match we could try other tutors. We opted for Avery and she was a complete star, listened, made it simple, set good homework and gave my daughter her confidence back in her abilities and gave her the confidence to face Maths GCSE exams. Highly recommend Degree Gap and will be back again in September with my youngest embarking on her GCSEs"
  - "Aila|Student|Harry was amazing! He helped me shape my personal statement into something I was truly proud of, and thanks to his support, I got into my dream university! Couldn’t have done it without him–highly recommend to anyone applying to uni."
  - "Sarom|Parent of A-Level Student|Harry has been very helpful in tutoring my son for his economics A Level. He has lots of patience, keeps lessons interesting and is a good motivator. Needless to say his subject matter is expertise is excellent."
---
