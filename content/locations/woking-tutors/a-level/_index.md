---
title: "Woking A-Level Tuition | From £37/hr"
description: "Woking A-Level tutors for the Year 12 step up, working one-to-one online over Lessonspace and matched on the specification. From £37 an hour."
layout: "subject"
location: "Woking"
level: "A-Level"

hero_heading_line1: "Online A-Level Tutors for the Woking Year 12 Whose GCSE Method Has Run Out"
hero_heading_line2: ""
hero_lead: "A-Level rewards a different habit from the one that carried a student through Year 11, and Year 12 is where that shows up. Our Woking A-Level tutors work one-to-one online over Lessonspace, rebuilding the method before the mocks that set predictions."

grade_from: "C"
grade_to: "A"
grade_stat: "One parent describes their child moving from Cs to As, with help on university choices alongside the subject work."

first_lesson_eyebrow: "WHEN THE GCSE METHOD RUNS OUT"
first_lesson_heading: "Why a Woking A-Level Tutor Goes Back to Year 12 First"
first_lesson_context: "The call we take most often from Woking parents opens the same way. He worked hard for his GCSEs and something has changed. Usually nothing has changed except the demand: A-Level asks a student to pick the method themselves, with nothing in the question announcing which one, and that is a skill rather than a personality trait."
first_lesson_quote: "Degree Gap helped my kid go from Cs to As. They have a great selection of tutors who not only assist with the curriculum but also helped with university choices and application."
first_lesson_quote_name: "Alexander"
first_lesson_quote_role: "Parent"
first_lesson_quote_grade: "Cs to As, with university applications alongside"

tutor_strip_heading: "Woking A-Level tutors who remember which Year 12 topics bite"
tutor_strip_body: "Students at Woking College, St John the Baptist and Gordon's sit different specifications, which at this level decides most of the value. A lot of our tutors sat these papers recently enough to remember the wording, and every session runs one-to-one on Lessonspace. Ask for two profiles and see."

pathways_heading: "What Woking Students Do With Two Years of A-Levels"
pathways_lead: "The three routes that come up most in a Year 12 conversation at this end of Surrey."
pathways:
  - title: "Universities"
    body: >
      The University of Surrey sits up the road in Guildford and Royal Holloway is over at Egham, with the London institutions inside a short run from Woking station. Offers get made against predicted grades written in Year 13, months before anybody sits a paper.
  - title: "Degree Apprenticeships"
    body: >
      McLaren advertises degree apprentice roles at its Woking site, and the University of Surrey delivers degree apprenticeships for employers around the county. Each scheme publishes its own entry requirements, and the deadlines often fall well before the UCAS one.
  - title: "Career Pathways"
    body: >
      Woking's employer base is unusual for a town this size, with McLaren at the Technology Centre, WWF-UK's Living Planet Centre and Capgemini's Woking office. Ashford and St Peter's Hospitals recruits and trains across north Surrey, and the fast line into Waterloo puts London employers inside a commute.

reviews:
  - "Cat|Parent of A-Level Student|My 18 year old son had A-level Economics tutoring with Harry.  He was excellent, knew exactly what he needed to learn and was able to explain in terms which made it simple to understand.  The online platform was convenient and easy to use."
  - "Joanna|Parent of A-Level Student|The A level tutoring made such a difference to my son, who had left studying until the final hour, managing to turn E and U grades into 3 C grades. Amazing"
  - "Sureet|Parent of A-Level Student|Haneet has really helped my son in navigating his A level Physics all whilst being kind and supportive."
  - "Mahir|A-Level Student|Harry was an excellent teacher that helped me gain real grip of the complicated areas in my subjects like economics - I'd highly recommend for anyone looking to consistently perform at their best."
  - "Daljit|Parent of A-Level Student|My son gained almost two grades higher than jis year 13 mock paper. His tutor Jacob was excellent"
  - "Nick|Parent of A-Level Student|Sandra Sabu has been tutoring our son for Psychology A Level. Sandra has provided a solid base of tutoring outside of school lessons and helped him to improve his latest grades."
  - "Lucas|A-Level Student|I was having trouble with some areas of my economics a level course before getting in contact with Harry. He walked me through everything I needed help with, from content I was weak with to exam technique that I was lacking. His insight, examples and willingness to go the extra step really helped me to regain my confidence in economics, having achieved a predicted a* with his help."
  - "Dan|A-Level Student|Really helped me get through my A-levels and made a massive difference when helping me get into my first choice of university."
  - "Binh|A-Level Student|I was very fortunate enough to encounter Harry during my time taking A level economics. I really struggled a lot with long 30 marked essays as well as evaluation and learning the graphs as for at the time it was really hard and I didn’t understand. However the way how Harry taught me was super simple compared to other teachers from school. I really enjoyed my time when Harry taught me and I would highly recommend it to people!"
  - "Sarom|Parent of A-Level Student|Harry has been very helpful in tutoring my son for his economics A Level. He has lots of patience, keeps lessons interesting and is a good motivator. Needless to say his subject matter is expertise is excellent."
  - "J.O|A-Level Student|I was put into contact with Harry from a friend of mine who did economics in the year above me. And at first I was hesitant on getting a Tutor as I wasn’t sure if they were effective especially for the price they demand, but this wasn’t the case with Harry. He’s helped me massively with my essays, topics I didn’t understand beforehand and overall confidence with the subject. I highly recommend!"
  - "Alex|University Student|Can’t recommend The Degree Gap highly enough. Quick and helpful responses from Harry. He immediately matched me with a very competent tutor, who was a massive help in helping me exceed my target grade and achieve a first class degree at university."

faq_1_q: "Is Year 12 or Year 13 the better time to start?"
faq_1_a: "Year 12, and the gap between the two is bigger than most families expect. Predicted grades come out of Year 12 work and the first part of Year 13, and they reach universities in the autumn, so an hour a week starting in October counts twice over. Year 13 starts still move the final grade and plenty of families begin there. But by then the offers have usually been made against a number set earlier, and the work turns into repair rather than build."
faq_2_q: "How does tutoring help with the predicted grade UCAS sees?"
faq_2_a: "By moving the evidence the prediction rests on, which is mock performance plus teacher judgement. A tutor reads the actual script instead of the grade, finds where the marks went, and works on that under timed conditions until the next assessment looks different. It also makes the conversation with school more useful, because you are asking about specific questions rather than asking for a better number. None of this happens in a fortnight, so autumn of Year 13 is the outside edge of useful."
faq_3_q: "She got a grade 8 at GCSE and is struggling now. What happened?"
faq_3_a: "Nothing went wrong with your daughter. GCSE rewards recall and recognising the question type, and strong students get very good at both. A-Level stops signposting. The question no longer tells you which method it wants, the marks sit in explanation and evaluation rather than the answer line, and a paper can run to two and a half hours. Students who coasted on pattern-matching feel that first and hardest. The fix is teaching the choosing, not more content."
faq_4_q: "Can you help with a degree apprenticeship application as well as the grades?"
faq_4_a: "Yes. McLaren, the NHS trust running north Surrey's hospitals and the schemes delivered through the University of Surrey all publish their own entry requirements, and the deadlines usually sit ahead of the UCAS one. Tutors work on the grades that get an application read, and our founders talk families through the timing, the written application and how to keep the university route open in parallel. Tell us on the call if both routes are live."

sitemap:
  priority: 0.7
  changefreq: monthly
---
