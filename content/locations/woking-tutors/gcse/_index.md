---
title: "Woking GCSE Tutors | One-to-One | The Degree Gap"
description: "Woking GCSE tutors for the year the sixth form has to be applied for. One-to-one online tutoring over Lessonspace, matched on the board, from £37 an hour."
layout: "subject"
location: "Woking"
level: "GCSE"

hero_heading_line1: "Online GCSE Tutors Working With Woking Families on the Subject Grade the Offer Needs"
hero_heading_line2: ""
hero_lead: "Three of Woking's biggest secondaries stop at sixteen, so Year 11 grades decide where a student goes next as well as how they finish. Our Woking GCSE tutors work one-to-one online over Lessonspace, an hour a week, on the subjects a course asks about."

grade_from: "5"
grade_to: "6/7"
grade_stat: "One Year 10 student moved from a grade 5 in English to a 6/7 across six months of weekly lessons."

first_lesson_eyebrow: "THE YEAR YOU APPLY OUT"
first_lesson_heading: "How a Woking GCSE Tutor Works Back From the Offer"
first_lesson_context: "Start from the course and the year plans itself. A student who needs one subject grade for a place at Woking College is doing different work from a student who needs five solid passes to keep the options open, and the first session sorts out which of those is in front of us. Then it runs off a marked paper rather than a feeling."
first_lesson_quote: "My son, who is in Year 10, really struggled with English and was completely disengaged from the subject. However, after six months of working with Malvina, he has improved from a grade 5 to a 6/7.\n\nI honestly can’t believe I’m saying this, but he now looks forward to his lessons. Through one-to-one tutoring, he has learned effective strategies that help him approach English in a way that actually resonates with his mathematical mindset.\n\nThank you to Degree Gap Tutoring!"
first_lesson_quote_name: "Omo"
first_lesson_quote_role: "Parent of GCSE Student"
first_lesson_quote_grade: "Grade 5 to Grade 6/7 in English"

tutor_strip_heading: "Woking GCSE tutors who work to the course your child is applying for"
tutor_strip_body: "Woking High, Winston Churchill and St John the Baptist do not all teach the same board, so we check the specification before a name goes out. Each tutor has come through a founder-led interview that about 3% of applicants pass. Have a look, or let us match your child."

pathways_heading: "Where Woking Year 11s Land Next September"
pathways_lead: "Three routes, and most families here are weighing up at least two of them by Christmas."
pathways:
  - title: "Sixth Forms"
    body: >
      Woking College takes a large share of the town's A-Level students, while St John the Baptist runs its own sixth form and Gordon's at West End takes day and boarding students into Year 12. Each publishes requirements course by course, with Maths and the Sciences generally naming a grade in the subject itself.
  - title: "Apprenticeships"
    body: >
      McLaren runs apprenticeships from its Technology Centre in Woking, across areas like manufacturing, vehicle development and IT, and Ashford and St Peter's Hospitals takes on apprentices across north Surrey. Maths and English are named in nearly all of them, and applications tend to open months before results day.
  - title: "Further Education"
    body: >
      Brooklands College runs T Levels and other Level 3 courses from its Weybridge and Ashford campuses, and Guildford College adds technical routes and BTECs a short way down the road. Both also teach GCSE English and Maths for students who need to sit either again.

reviews:
  - "Keira|GCSE Student|Has definitely helped me increase my grades from an E to a B!!"
  - "Ellen|Parent of GCSE Student|My daughter was struggling with confidence in Maths GCSE and we decided to get a tutor. After lots of recommendations we booked a chat with Harry who listened to my daughters needs and gave a selection of tutors to choose from. He made it clear that if it didn’t feel like a good match we could try other tutors. We opted for Avery and she was a complete star, listened, made it simple, set good homework and gave my daughter her confidence back in her abilities and gave her the confidence to face Maths GCSE exams. Highly recommend Degree Gap and will be back again in September with my youngest embarking on her GCSEs"
  - "Augusta|Parent of GCSE Student|The support given to my son by The Degree Gap's tutors was fabulous and he passed all his GCSE subjects. The Degree Gap provide experienced and organised tutors. To improve my son's grades, the tutors provided resources, targeting specific areas of difficulties. The website is easy to use and effective, I had no disappointment in scheduling lessons for my son. Tutors were polite and on time for their lessons."
  - "Bryan|GCSE Student|Helped me a lot and really showed me what i need to improve on"
  - "Dawn|Parent|The Degree Gap took time to understand my child’s worries. They paired me with Connor, who my daughter got on well with. Her confidence has skyrocketed since. Thanks again."
  - "Philippa|Parent of GCSE Student|I was really pleased to discover the Degree Gap when my daughter needed help getting the grades she needed for GCSEs. The team offer a really personalised service and I felt really well looked after and supported from the start. There are lots of tutors to choose from and all subjects are catered for, with great flexibility on timings to fit around school lessons. The tutors we have used are lovely.  The Degree Gap use an advanced online platform which enables interactive tuition. We also attended a free webinar from the Degree Gap which was very informative and added value. I highly recommend them."
  - "Nazrul|Parent|My daughter found the tutors were very helpful for her GCSE thank you"
  - "Isabel|Parent of iGCSE Student|Harry helped our son enormously with his maths and economics iGCSE prep. He was patient, fun and engaging - just what we needed. Highly recommended."
  - "Jessica|Student|I highly recommend Harry – he was excellent in helping me with both Maths and English for my GCSEs. Thanks to his support, I passed both subjects with great results, which gave me the foundation and confidence to go on to succeed in my A-levels and complete my degree!!"
  - "Anders|iGCSE Student|I reached out to The Degree Gap to get some help to prepare for my iGCSE exams and was very impressed by their tutors and professionalism!"
  - "Sorland|Grandparent|My Granddaughter uses the Degree Gap tutoring services across a few subjects. She is not only improving academically but really enjoys the setup and structure too. I would recommend to anyone."
  - "Charlotte|Student|I cannot recommend Harry enough. He was helpful welcoming and helped me achieve a grade higher than I was expected!"

faq_1_q: "Year 10 or Year 11, when does GCSE tutoring make sense in Woking?"
faq_1_a: "Year 10 if you have the choice. The content steps up that year and a gap closed then stops doing damage for another eighteen months. There is a local reason too: college and sixth form applications open in the autumn of Year 11, and the mock sitting behind the predicted grade on that form happens soon after. Plenty of families begin in Year 11 and it still works, but by then the plan narrows towards papers and technique rather than teaching anything properly from the beginning."
faq_2_q: "Do your tutors cover the boards Woking schools teach?"
faq_2_a: "Yes, and it is the first thing we pin down rather than a detail left until later. AQA, Edexcel, OCR and WJEC word questions differently, examine practicals differently and print different formulae, so a tutor who knows your child's specification is useful inside ten minutes. We confirm the board on the consultation call, before any profile goes out. If your child is on an iGCSE at an independent school, say so, because the paper mix changes again."
faq_3_q: "He is fine in class and drops marks in mocks. What can a tutor actually do about that?"
faq_3_a: "Read the script, which is the bit nobody at school has time for. Lost marks cluster, and where they cluster tells you whether this is content, timing or the way answers get written down. A student who works calmly at home and loses the last twenty minutes of a paper needs drilling under a clock, not more revision. That work is hard to do alone and it moves quickly, usually inside a term of weekly hours."
faq_4_q: "What does it cost, and are we tied into anything?"
faq_4_a: "From £37 an hour, agreed with you before the first lesson, and the rate depends on how experienced the tutor is. That covers the online session over Lessonspace, the prep, the resources and the note afterwards. No joining fee, no materials charge, no contract. You pay one session at a time and can pause in half term or stop altogether, which some families do once a grade has settled and then pick up again before exams."

sitemap:
  priority: 0.7
  changefreq: monthly
---
