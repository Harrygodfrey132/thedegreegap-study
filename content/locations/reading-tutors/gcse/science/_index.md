---
title: "Reading GCSE Science Tutors | Online | The Degree Gap"
nav_title: "Science"
description: "Online GCSE Science tutors covering Reading for triple and combined award. Exam-question practice over flashcards, required practicals, from £37 an hour."
layout: "level-subject"
location: "Reading"
level: "GCSE"
subject: "Science"

hero_h1: "Online GCSE Science Tutors Covering Reading"
hero_lead: "Flashcards prove a student can recall a definition, and the paper almost never asks for one. Our Reading GCSE Science tutors work online on the questions themselves, which is where the marks are."

angle_eyebrow: "RECALL IS NOT THE THING BEING TESTED"
angle_heading: "Why Science Revision Often Misses What the Paper Asks For"
angle_body_1: "Science looks like the most revisable subject at GCSE, which is exactly the trap. There is a lot of content, so students make flashcards, and flashcards test recall. But the paper rarely asks a student to state a definition. It asks them to apply an idea to a situation they have not seen, to explain a chain of cause and effect across six marks, to evaluate a method, or to work out what would happen if one variable changed. A student can know every definition on the specification and still lose a third of the paper, which is why weeks of visible revision sometimes produce a grade that has not moved."
angle_body_2: "The fix is to make the revision look like the exam, which means past questions marked against the real scheme rather than content covered again. It is less comfortable, because it shows what is not working, and it is considerably more efficient. The first hour does exactly that with a recent paper and shows your child where the marks actually went. Reading students split across several routes at sixteen, from grammar and school sixth forms to Reading College and the tech employers out at Thames Valley Park, and each publishes its own Science requirements, revised between years."
angle_image: "/images/fluorescent-cells-under-microscope.jpg"
angle_image_alt: "A GCSE Science student working through a required practical question with an online tutor"
angle_stat_from: "5"
angle_stat_to: "7"
angle_stat_detail: "The kind of movement a term of weekly one-to-one Science hours usually targets once revision is built around exam questions."

schools:
  - "Prospect School"
  - "The Bulmershe School"
  - "John Madejski Academy"
  - "Reading Blue Coat School"
  - "The Abbey School"
  - "Little Heath School"

steps_heading: "Start online GCSE Science tutoring in Reading in 3 steps"
steps_lead: "Profiles inside a day. If your child has been revising hard without the grade moving, that is usually a method problem rather than an effort one."
steps:
  - title: "Share your goals"
    body: "We need the year group, the school, the exam board and the Science route. Tell us how your child currently revises Science, because that answer often explains the gap on its own. Which of the three feels worst is the other thing worth saying."
  - title: "Meet your match"
    body: "A shortlist of two or three arrives the following day, narrowed by specification and by which of the three is behind. You get a free 15-minute video meeting with them before committing, so nobody commits before they have met."
  - title: "Start the lessons"
    body: "Weekly hour-long sessions on Lessonspace, where tutor and student write on the same screen. Real exam questions from the start, marked against the real scheme in front of your child, with the method written out rather than described. Missing content gets taught properly. Sessions are recorded. £37 an hour upwards, paid one session at a time."

faqs:
  - q: "What grade do you need for A-Level Sciences at Reading sixth forms?"
    a: "Every provider around Reading sets its own bar and revises it between years, and the Sciences generally sit near the top of what any of them asks for. The grammar sixth forms, the school sixth forms and Reading College all publish their own requirements and they are not the same, with some wording it differently depending on whether a student sat triple or combined. Read the page for the specific place rather than working from a general figure. Where a student is a grade short, twelve to sixteen weeks of weekly hours is a realistic window."
  - q: "What is the difference between AQA, Edexcel and OCR GCSE Science, and does it matter for tutoring?"
    a: "Yes, and in Science it matters more than in most subjects. Content overlaps heavily while examining does not: the boards assess required practicals differently, mark six-mark answers to different structures, and print different amounts of equation support in the paper. A student who met a practical under one board's method can meet it framed another way and lose marks on material they genuinely know. We confirm the board on the consultation call, and where a student has changed schools we look for a tutor who has taught both."
  - q: "How do I know if my child needs a Science tutor or just more revision?"
    a: "Look at how they revise before deciding. If the revision is flashcards, notes and videos and the grade has not moved in two terms, more of it will not help and a tutor changes the method rather than the volume. If your child is already working through past papers and marking them honestly but stalls on the six-mark questions or the calculations, a tutor is the fastest fix because both are teachable habits. If they are doing neither and simply have not started, the first thing to try is structured past-paper practice, and we will say so."
  - q: "How do you vet your GCSE Science tutors?"
    a: "The pass rate sits around 3%. We ask for a written application, test subject knowledge, and then interview, and it is the interview that does the work. What is being judged there is not how much someone knows but whether they can tell when a student has stopped following and change tack. Joe and Harry do this personally. It is slow and it is the reason we can usually match a family properly on the first attempt rather than the second."
  - q: "How much does GCSE Science tutoring cost in Reading, and what is included?"
    a: "From £37 an hour, with the rate depending on the tutor's experience and agreed before anything is booked. The hour covers the session, the preparation behind it, the practice material, the marking and a short note afterwards on what was covered and set. There is no joining fee, no materials charge and no contract, and sessions are paid one at a time. The consultation call and the 15-minute meeting with your chosen tutor both happen before you pay anything at all."
# FAQ picks: G01, A02, A10, E01, C02

reviews:
  - "Jeanne|Student|I can’t recommend The degree gap enough! Harry provided quick and extremely helpful feedback on my personal statement for my master’s applications. Thank you, Harry!"
  - "Darren|Parent|A really helpful experience for supporting our Son.Thank you Joe,Harry and Matt."
  - "Joel|Student|The degree gap is an amazing place and have helped me achieve so much and I am looking forward to it helping it achieve a higher class of skills and ready for the RAF"
  - "joe|Student|Highly recommend the Degree Gap as a tutoring agency! Their platform is easy to use and having used tutors across a variety of subjects, they have all been consistently excellent."
  - "Kasia|Parent|Harry is a great tutor. He's very helpful and knowledgeable. He's git goid understanding of student's needs as well as current situation when it comes to university education in the UK. His experience and expertise helped my son make some decisions about choosing A level subject and university course. I highly recommend Harry and Degree Gap."
---
